...
/* THERE IS A DIFFERENCE BETWEEN SLIDES AND MANUAL CHECK IF ERRORS GENERATED*/
...
typedef union page_directory_entry_4KB_t {
    ...
    struct {
        ...
        uint8_t accessed : 1;
        uint8_t available_bit : 1; // reserved so set to 0
        uint8_t page_size : 1; // 0 for 4 KiB
        uint8_t global : 1;
        uint8_t available : 3; // available for system programmer
...
typedef union page_directory_entry_4MB_t {
    ...
    struct {
        ...
        uint8_t cache_disable : 1;
        uint8_t accessed : 1;
        uint8_t dirty : 1; // reserved so set to 0
        uint8_t page_size : 1; // 0 for 4 KiB
        uint8_t global : 1;
        uint8_t available : 3; // available for system programmer
        uint8_t page_table_attribute_index : 1;
        uint32_t reserved : 9;
        uint32_t page_base_address : 10; // page table address
    } __attribute__ ((packed));
} page_directory_entry_4MB_t;

typedef union page_directory_entry_t {
    page_directory_entry_4KB_t p4KB;
    page_directory_entry_4MB_t p4MB;
...
typedef union page_table_entry_t {
    ...
    struct {
        ...
        uint8_t accessed : 1;
        uint8_t dirty : 1;
        uint8_t page_attribute_table : 1; // 0 ?
        uint8_t global : 1;
        uint8_t available : 3;
        uint32_t address : 20;
    } __attribute__ ((packed));
...
page_directory_entry_t page_directory[NUM_ENTRY] __attribute__((aligned(NUM_BYTES)));
page_table_entry_t page_table[NUM_ENTRY] __attribute__((aligned(NUM_BYTES)));
page_table_entry_t vid_page_table[NUM_ENTRY] __attribute__((aligned(NUM_BYTES)));
